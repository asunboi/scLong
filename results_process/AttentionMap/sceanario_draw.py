...
colors = {'SCINA-signature': 'purple', 'marker genes': 'red'} 
# Create a second legend for categories
category_patches = [mpatches.Patch(color=color, label=cat) for cat, color in colors.items()]
second_legend = plt.legend(handles=category_patches, loc='upper right', bbox_to_anchor=(0.006, -0.08), title = 'Special genes', fontsize = 6, title_fontsize=6)
#plt.setp(second_legend.get_title(),fontsize=16)
plt.tight_layout()
plt.savefig(f'sceanario_title_genes.pdf', dpi = 120)
plt.close()