...
class celltype_GRN_model:
    ...
    def initalize_A(self,data):
        ...
        #exit()
        if scfm:
            #A = np.load('/data1/ruiyi/scfm/adj_deepsem_scfm.npy')[1:,1:]
            A = np.load('/data3/ruiyi/deepsem/adj_deepsem_scfm_nopre.npy')
            for i in range(num_genes):
                if sum(A[i])==0:
                    A[i]=np.ones(num_genes)/ (num_genes - 1)
        else:
    ...
    def init_data(self,):
        ...
        if os.path.exists('/data3/ruiyi/deepsem/truth_df.pkl'):
            
            truth_df=pd.read_pickle("/data3/ruiyi/deepsem/truth_df.pkl") 
        else:
            truth_df = pd.DataFrame(np.zeros([num_genes, num_genes]), index=data.columns, columns=data.columns)
            for i in tqdm(range(Ground_Truth.shape[0])):
                truth_df.loc[Ground_Truth.iloc[i, 1], Ground_Truth.iloc[i, 0]] = 1
            truth_df.to_pickle('/data3/ruiyi/deepsem/truth_df.pkl')
    ...
    def train_model(self):
        ...
        for epoch in range(self.opt.n_epochs+1):
            ...
            self.opt.save_name + '/GRN_inference_result_ep{}.tsv'.format(str(epoch)), sep='\t', index=False)
