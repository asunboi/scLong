...
parser.add_argument('--scfm',action='store_true')
...
model = DeepCE(drug_input_dim=drug_input_dim, drug_emb_dim=drug_embed_dim,
                      conv_size=conv_size, degree=degree, gene_input_dim=gene_input_dim,
...
#scfm_embed=torch.cat((torch.DoubleTensor(np.load('/data1/ruiyi/deepce/deepce_scfm_gene2vec_embed.npy')).to(device),data.gene),dim=-1)
#scfm_embed=torch.DoubleTensor(np.load('/data1/ruiyi/deepce/deepce_scfm_0131_gene2vec_embed.npy')).to(device)
#scfm_embed=torch.DoubleTensor(np.load('/data1/ruiyi/deepce/deepce_scfm_10m_gene2vec_embed.npy')).to(device)
#scfm_embed=torch.DoubleTensor(np.load('/data1/ruiyi/deepce/deepce_geneformer_gene2vec_embed.npy')).to(device)
scfm_embed=torch.DoubleTensor(np.load('/data1/ruiyi/deepce/deepce_scgpt_gene2vec_embed.npy')).to(device)

print(scfm_embed.shape)
#scfm_embed=torch.randn((np.shape(data.gene)[0],200),dtype=torch.double).to(device)
# training
...
for epoch in range(max_epoch):
    ...
    for i, batch in enumerate(data.get_batch_data(dataset='train', batch_size=batch_size, shuffle=True)):
        ...
        optimizer.zero_grad()
        if args.scfm:
            predict = model(drug, scfm_embed, mask, pert_type, cell_id, pert_idose)
        else:
